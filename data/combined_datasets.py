...
from data import CheXpertDataset, MimicCxrJpgDataset
...
class CombinedXrayDataset(BaseDataset):
    ...
    Args:
        ...
        split_list: List of strings specifying split. If a string is passed
            (e.g., 'train'), that split will be broadcast to all
            sub-dataloaders.
            options include:
    ...
    def __init__(self, directory_list: List[Union[str, os.PathLike]], dataset_name: str,
    ...
    @staticmethod
    def fetch_label_list(dataset_name_list: List[str]) -> List[str]:
        label_list: List[str] = []
        for dataset_name in dataset_name_list:
            if dataset_name == "chexpert_v1":
                label_list += CheXpertDataset.default_labels()
            elif dataset_name == "mimic-cxr":
                label_list += MimicCxrJpgDataset.default_labels()
